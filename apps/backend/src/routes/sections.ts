...
router.get("/", async (req: AuthRequest, res) => {
  ...
  const check = await pool.query("SELECT id FROM projects WHERE id = $1 AND owner_id = $2", [projectId, req.userId]);
  if (!check.rows.length) {
    res.status(404).json({ error: "Project not found" });
    return;
  }
  const result = await pool.query("SELECT id, project_id, name, sort_order FROM sections WHERE project_id = $1 ORDER BY sort_order ASC", [projectId]);
